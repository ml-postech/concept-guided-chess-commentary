...
class TFProcess:
    ...
    def init_net(self, return_activations=False):
        self.l2reg = tf.keras.regularizers.l2(l2=0.5 * (0.0001))
        input_var = tf.keras.Input(shape=(112, 8, 8))
        if return_activations:
            outputs, out_activation = self.construct_net(input_var, return_activations=return_activations)
            self.model = tf.keras.Model(inputs=input_var, outputs=outputs)
            self.activation_extractor = tf.keras.Model(inputs=input_var, outputs=out_activation)
        else:
    ...
    def create_residual_body(self, inputs, return_activations=False):
        activations = []
        ...
        for i in range(self.RESIDUAL_BLOCKS):
            ...
            activations.append(flow)    
        if return_activations:
            return flow, activations
        else: 
            return flow

    def create_encoder_body(self, inputs, embedding_size, return_activations=False):
        # Policy head
        ...
        for i in range(self.encoder_layers):
            ...
            activations.append(flow) 
        if return_activations:
            return flow, attn_wts, activations
    ...
    def construct_net(self, inputs, name='', return_activations=False):   #TODO: add return_activation

        if return_activations:
            if self.encoder_layers > 0:
                flow, attn_wts, activations = self.create_encoder_body(inputs,
                                                        self.embedding_size, return_activations)
            else:
                flow, activations = self.create_residual_body(inputs, return_activations)
        else:
            if self.encoder_layers > 0:
        ...
        if return_activations:
            activations.append(h_conv_val_flat)
        if self.wdl:
