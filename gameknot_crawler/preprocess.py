...
extra_links=pickle.load(open("saved_files/extra_pages.p","rb"))
...
for i,link in all_links:
    ...
    startState[1]="rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -|||"
    ...
    for pageNo in range(pageLength):
        ...
        for elem in data:
            ...
            moves=elem[0]
            moves=moves.split('\n')[0]
            if '12345' in moves or '8765' in moves:
                print("move skipped")
                continue
            board=elem[1]
            comment = elem[2].strip()
            commentWords=" ".join(word_tokenize(comment))
            currentStateString=board.split("|||")[0]
            startStateString=startState[1].split("|||")[0]
            diffStateString=board.split("|||")[1]
